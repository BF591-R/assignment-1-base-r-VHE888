...
less_than_zero_sum <- function(x, na.rm=TRUE) {
  return(sum((x < 0)))
}

btw_1_and_5_sum <- function(x, na.rm=TRUE) {
  return(sum((x > 1) & (x < 5)))
}

rm_na_sum <- function(x) {
  return(sum(is.na(x)))
}
...
summarize_matrix <- function(x, na.rm=TRUE) {
    return(data.frame(
      ...
      min = apply(x, 1, min),
      max = apply(x, 1, max),
      num_lt_0 = apply(x, 1, less_than_zero_sum),
      num_btw_1_and_5 = apply(x, 1, btw_1_and_5_sum),
      num_na = apply(x, 1, rm_na_sum)
    ))
}
